---
description: "Create stacked PRs with auto-detected branch structure"
user-invocable: true
...
```bash
# Create PR with base branch — always draft
gh pr create \
  --draft \
  --base "$ARGUMENTS" \
...
```bash
source ${CLAUDE_PLUGIN_ROOT}/scripts/stack-scripts.sh
